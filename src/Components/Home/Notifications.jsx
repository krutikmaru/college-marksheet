...
const Notifications = () => {
  const { course, setCourse } = useDataStore();
  const { user } = useUser();
  ...
  const handlePublish = (pendingPublish) => {
    ...
    copy.data[pendingPublish.batch][pendingPublish.course][
      ...
      "published";
    setCourse(copy);
    toast.success(`Published ${pendingPublish.abbrevation} successfully`);
  };
              ...
              >
                ...
                <div>
                  <button
                    ...
                    onClick={() => handlePublish(pendingPublish)}
                  >
                    <FontAwesomeIcon
...
const PublishNotification = ({ publish }) => {
  return <div></div>;
};
